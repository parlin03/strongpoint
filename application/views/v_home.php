												...
												<div class="row">
													...
													<div class="col-md-4">
														<div class="table-responsive">
															<table class="table table-bordered table-striped table-hover text-dark  ">
																<thead class="text-center">
																	<th class="border">Perusahaan</th>
																	<th class="border">Jumlah</th>
																	<!-- <th class="border">REKOMENDASI</th> -->
																	<!-- <th class="border">Action</th> -->
																</thead>
																<tbody>
																	<?php if (empty($summaryPerusahaan)) : ?>
																		<tr>
																			<td colspan="7">
																				<div class="alert alert-danger" role="alert">
																					data not found!
																	...
																	<?php
																	$jtotal = 0;
																	foreach ($summaryPerusahaan as $row) : ?>
																		<?php $jtotal += $row->total; ?>
																		<tr class="text-center">

																			<td class="border"><a href="<?= base_url('pekerjaan/') . strtolower($row->perusahaan); ?>"><?= $row->perusahaan; ?></a></td>
																			<td class="border"><?= "Rp " . number_format("$row->total", 0, ",", ".") . " (" . number_format("$row->percentage", 2, ",", ".") . "%)"; ?></td>
...
<script type="text/javascript">
	$(document).ready(function() {
		var options = {
			...
			title: {
				text: 'Sebaran Pagu Anggaran Perusahaan'
			},
			tooltip: {
				formatter: function() {
					return '<b>' + this.point.name + '</b>: ' + this.percentage + ' %';
		...
		$.getJSON("<?php echo site_url('home/perusahaan_list'); ?>", function(json) {
			options.series[0].data = json;
			chart = new Highcharts.Chart(options);
		});
