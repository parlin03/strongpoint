...
class Dpt extends MY_Controller
{
    ...
    */
    function index()
    {
        ...
        #$data['table_list'] = $this->import_model->fetch_dpt();
        ...
        // config pagination
        $config['base_url'] = base_url() . 'import/dpt';
        $config['total_rows'] = $this->dpt->countAllDpt1($data['keyword']);
        ...
        $data['table_list'] = $this->dpt->getDpt1Kecamatan($config['per_page'], $data['start'], $data['keyword']);

        $this->load->helper('url');       //pointer sidebar
        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('import/dpt/content', $data);
    ...
    */
    function import_excel()
    {
        ...
        if (isset($_FILES['uploadFile']['name']) && in_array($_FILES['uploadFile']['type'], $file_mimes)) {
            ...
            if ($array_data != '') {
                $this->import_model->insert_dpt_batch($array_data);
            }
